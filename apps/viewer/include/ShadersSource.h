...
const std::string wireframeVert = R"(
#version 410 core
layout (location = 0) in vec3 inPosition;
...
const std::string wireframeFrag = R"(
#version 410 core
in vec3 barycenter;
...
// 仅在指定半径范围内绘制圆环
			...
			if (r >= innerR && r <= outerR) {
				// 计算距离中心的环数（从内向外）
				float ringDistance = r - innerR;
				float ringNumber = ringDistance / ringSpacing;
    
				// 计算当前位置在环内的相对位置（0-1范围）
				float ringPos = fract(ringNumber);
    
				// 绘制环的边界（类似网格线）
				if (ringPos < 0.05 || ringPos > 0.95) {
					a = 1.0;
				}
			}
	...
	if (shade == 1) {
		...
		fragColor = vec4((0.5*diffuse + 0.4*specular + 0.6*fresnel + ambient)*rgb, 1.0);
