...
TextBox* Viewer::patternSizeTextBox = NULL;
...
float Viewer::innerR = 0.1;
float Viewer::outerR = 0.2;
float Viewer::ringSpacing = 0.1;
...
//std::shared_ptr<RenderMesh> etcherMesh = nullptr;
//glm::mat4 etcherTransform = glm::mat4(1.0f);
...
void Viewer::initGLFW()
{
	...
	// initialize window
	window = glfwCreateWindow(windowWidth, windowHeight, "bff", NULL, NULL);
	if (!window) {
...
void Viewer::initGui()
{
	...
	Window* ModelIO = new Window(gui, "Model IO");
	// io
	Button* loadMeshButton = new Button(ModelIO, "Load Mesh");
	...
	Button* exportMeshButton = new Button(ModelIO, "Export UVs");
	...
	exportNormalizedUVsCheckBox = new CheckBox(ModelIO, "Normalize UVs to [0, 1]");
	exportNormalizedUVsCheckBox->setFixedHeight(14);

	useVtFlagToStoreUVsCheckBox = new CheckBox(ModelIO, "Use 'vt' flag to store UVs");
	useVtFlagToStoreUVsCheckBox->setFixedHeight(14);
	ModelIO->setLayout(new GroupLayout());
	ModelIO->setVisible(false);   // 默认隐藏
	ModelIO->setPosition(Vector2i(250, 50));
	ModelIO->setFixedWidth(180);
	Window* toolChest = new Window(gui, "Tool Chest");
	toolChest->setLayout(new GroupLayout());

	Button* ModelIOBtn = new Button(toolChest, "ModelIO");
	ModelIOBtn->setFixedHeight(25);
	// Flattening Module Button
	Button* flattenBtn = new Button(toolChest, "Flattening Module");
	flattenBtn->setFixedHeight(25);

	
	// LaserEtcher Button
	Button* LaserEtcherBtn = new Button(toolChest, "Add Laser Etcher");
	LaserEtcherBtn->setFixedHeight(25);

	//Pattern Generator Button
	Button* patGenBtn = new Button(toolChest, "PatGen");
	patGenBtn->setFixedHeight(25);

	Window *nanoguibff = new Window(gui, "flatten module");
	nanoguibff->setLayout(new GroupLayout());
	nanoguibff->setVisible(false);   // 默认隐藏
	nanoguibff->setPosition(Vector2i(250, 50));
	...
	// plot
	plotLabel = new Label(nanoguibff, "Plot", "sans");
	ComboBox *plotComboBox = new ComboBox(nanoguibff, {"      Constant      ",
														  "       Shaded       ",
														  "       Normal       ",
	...
	// wireframe
	CheckBox *wireframeCheckBox = new CheckBox(nanoguibff, "Show Wireframe");
	...
	// target boundary
	new Label(nanoguibff, "Target Boundary", "sans");
	targetBoundaryComboBox = new ComboBox(nanoguibff, {"     Automatic     ",
														  "        Disk       ",
														  "   Edit Boundary   ",
														  "Set Boundary Angles"});
	...
	// cones
	angleControlLabel = new Label(nanoguibff, "Angle Control             Sum:0π", "sans");
	angleTextBox = new TextBox(nanoguibff);
	...
	angleSlider = new Slider(nanoguibff);
	angleSlider->setRange(std::make_pair(-4.0, 1.0));
	...
	// cone placement
	new Label(nanoguibff, "Automatic Cone Placement (slow)", "sans");
	vertexHandleCountTextBox = new TextBox(nanoguibff);
	...
	placeConesButton = new Button(nanoguibff, "Place Cones");
	...
	Button *removeSeamsButton = new Button(nanoguibff, "Remove Seams");
	removeSeamsButton->setFixedHeight(25);
	removeSeamsButton->setEnabled(false);
	

	new Label(nanoguibff, "Spherical Parameterization");
	Button *sphericalParameterizationButton = new Button(nanoguibff, "Map to Sphere");
	sphericalParameterizationButton->setFixedHeight(25);
	sphericalParameterizationButton->setCallback([] { mapToSphere(); });

	new Label(nanoguibff, "");
	Button *resetButton = new Button(nanoguibff, "Reset");
	...
	// help text
	instructionText = new Label(nanoguibff, "", "sans", 15);
	instructionText->setFixedSize(Eigen::Vector2i(190, 60));
	instructionText->setColor(highlightColor);


	Window* LaserEtcherWin = new Window(gui, "Laser Etcher");
	LaserEtcherWin->setLayout(new GroupLayout());
	LaserEtcherWin->setVisible(false);   // 默认隐藏
	LaserEtcherWin->setPosition(Vector2i(500, 50)); // 弹出位置

	new Label(LaserEtcherWin, "Laser Etcher Controls", "sans-bold");

	// 位置控制
	new Label(LaserEtcherWin, "Position X", "sans");
	posXSlider = new Slider(LaserEtcherWin);
	posXSlider->setRange({ -5.0, 5.0 });
	...
	new Label(LaserEtcherWin, "Position Y", "sans");
	posYSlider = new Slider(LaserEtcherWin);
	posYSlider->setRange({ -5.0, 5.0 });
	...
	new Label(LaserEtcherWin, "Position Z", "sans");
	posZSlider = new Slider(LaserEtcherWin);
	posZSlider->setRange({ 0.5, 10.0 });  // 保证在模型上方
	...
	// 旋转控制
	new Label(LaserEtcherWin, "Rotation X (±30°)", "sans");
	rotXSlider = new Slider(LaserEtcherWin);
	rotXSlider->setRange({ -etcher.maxTiltX, etcher.maxTiltX });
	...
	new Label(LaserEtcherWin, "Rotation Y (±30°)", "sans");
	rotYSlider = new Slider(LaserEtcherWin);
	rotYSlider->setRange({ -etcher.maxTiltY, etcher.maxTiltY });
	...
	new Label(LaserEtcherWin, "Rotation Z (±60°)", "sans");
	rotZSlider = new Slider(LaserEtcherWin);
	rotZSlider->setRange({ -etcher.maxTiltZ, etcher.maxTiltZ });
	...
	Window* patGenWin = new Window(gui, "PatGen");
	patGenWin->setLayout(new GroupLayout());
	patGenWin->setVisible(false);   // 默认隐藏
	patGenWin->setPosition(Vector2i(500, 50)); // 弹出位置

	// check size
	patternComboBox = new ComboBox(patGenWin, { "     None     ",
												   "     Grid     ",
												   " Checkerboard ",
												   "    Circles   ",
												   "    Rings     " });
	...
	new Label(patternSizePanel, "Pattern Size");
	patternSizePanel->setLayout(new BoxLayout(Orientation::Horizontal, Alignment::Middle, 5, 5));

	Slider* patternSizeSlider = new Slider(patternSizePanel);
	patternSizeSlider->setRange(std::make_pair(0.0f, 5.0f));
	patternSizeSlider->setValue(patternSize);
	patternSizeSlider->setFixedHeight(14);

	TextBox* patternSizeTextBox = new TextBox(patternSizePanel);
	{
		std::ostringstream ss;
		ss.precision(4);
		ss << std::fixed << patternSize;
		patternSizeTextBox->setValue(ss.str());
	}
	patternSizeTextBox->setEditable(true);
	patternSizeTextBox->setAlignment(TextBox::Alignment::Right);
	patternSizeTextBox->setFixedWidth(80);
	...
	// ---------- 文本框 -> 滑条 ----------
	patternSizeTextBox->setCallback([patternSizeSlider, patternSizeTextBox](const std::string& text) -> bool {
		try {
	...
	Widget* innerRPanel = new Widget(patGenWin);
	innerRPanel->setLayout(new BoxLayout(Orientation::Horizontal, Alignment::Middle, 5, 5));
	new Label(innerRPanel, "Inner R:", "sans-bold");

	Slider* innerRSlider = new Slider(innerRPanel);
	innerRSlider->setRange(std::make_pair(0.0f, outerR - 0.0001f));  // 上界为outerR当前值
	innerRSlider->setValue(innerR);
	innerRSlider->setFixedHeight(14);

	TextBox* innerRBox = new TextBox(innerRPanel);
	innerRBox->setValue(floatToString(innerR, 4));
	innerRBox->setEditable(true);
	innerRBox->setAlignment(TextBox::Alignment::Right);
	innerRBox->setFixedWidth(80);

	// ========================================
	// Outer R 控件
	Widget* outerRPanel = new Widget(patGenWin);
	outerRPanel->setLayout(new BoxLayout(Orientation::Horizontal, Alignment::Middle, 5, 5));
	new Label(outerRPanel, "Outer R:", "sans-bold");

	Slider* outerRSlider = new Slider(outerRPanel);
	outerRSlider->setRange(std::make_pair(innerR + 0.0001f, 5.0f));  // 下界为innerR当前值，上界固定为10
	outerRSlider->setValue(outerR);
	outerRSlider->setFixedHeight(14);

	TextBox* outerRBox = new TextBox(outerRPanel);
	outerRBox->setValue(floatToString(outerR, 4));
	outerRBox->setEditable(true);
	outerRBox->setAlignment(TextBox::Alignment::Right);
	outerRBox->setFixedWidth(80);

	// ========================================
	// 双向联动逻辑
	innerRSlider->setCallback([innerRBox, outerRSlider](float v) {
		innerR = clampValue(v, 0.0f, outerR - 0.0001f);  // 限制在0到outerR之间
		innerRBox->setValue(floatToString(innerR, 4));

		// 更新 outerR 的下界为当前innerR
		outerRSlider->setRange(std::make_pair(innerR + 0.0001f, 5.0f));
		});

	innerRBox->setCallback([innerRSlider, innerRBox, outerRSlider](const std::string& text) -> bool {
		try {
			float v = std::stof(text);
			innerR = clampValue(v, 0.0f, outerR-0.0001f);  // 限制在0到outerR之间
			innerRSlider->setValue(innerR);
	...
	outerRSlider->setCallback([outerRBox, innerRSlider](float v) {
		outerR = clampValue(v, innerR + 0.0001f, 5.0f);  // 限制在innerR到10之间
		outerRBox->setValue(floatToString(outerR, 4));

		// 更新 innerR 的上界为当前outerR
		innerRSlider->setRange(std::make_pair(0.0f, outerR - 0.0001f));
		});

	outerRBox->setCallback([outerRSlider, outerRBox, innerRSlider](const std::string& text) -> bool {
		try {
			float v = std::stof(text);
			outerR = clampValue(v, innerR + 0.0001f, 5.0f);  // 限制在innerR到10之间
			outerRSlider->setValue(outerR);
	...
	Widget* ringSpacingPanel = new Widget(patGenWin);
	new Label(ringSpacingPanel, "ringSpacing");
	ringSpacingPanel->setLayout(new BoxLayout(Orientation::Horizontal, Alignment::Middle, 5, 5));

	Slider* ringSpacingSlider = new Slider(ringSpacingPanel);
	ringSpacingSlider->setRange(std::make_pair(0.0001f, 1.0000f));
	ringSpacingSlider->setValue(ringSpacing);
	ringSpacingSlider->setFixedHeight(14);

	TextBox* ringSpacingBox = new TextBox(ringSpacingPanel);
	ringSpacingBox->setValue(formatFloat(ringSpacing));
	ringSpacingBox->setEditable(true);
	ringSpacingBox->setAlignment(TextBox::Alignment::Right);
	ringSpacingBox->setFixedWidth(80);

	ringSpacingSlider->setCallback([ringSpacingBox](float v) {
		ringSpacing = v;
		std::ostringstream ss;
		ss.precision(4);
		ss << std::fixed << v;
		ringSpacingBox->setValue(ss.str());
		});

	// ---------- 文本框 -> 滑条 ----------
	ringSpacingBox->setCallback([ringSpacingSlider, ringSpacingBox](const std::string& text) -> bool {
		try {
			float v = std::stof(text);
			v = clampValue(v, 0.0f, 1.0f);
			ringSpacing = v;
			ringSpacingSlider->setValue(v);
	...
	ModelIOBtn->setCallback([ModelIO](void) {
		bool visible = !ModelIO->visible();
		ModelIO->setVisible(visible);
		ModelIO->requestFocus();
		});
	
	flattenBtn->setCallback([nanoguibff](void) {
		bool visible = !nanoguibff->visible();
		nanoguibff->setVisible(visible);
		nanoguibff->requestFocus();
		});

	LaserEtcherBtn->setCallback([LaserEtcherWin](void) {
		bool visible = !LaserEtcherWin->visible();
		LaserEtcherWin->setVisible(visible);
		LaserEtcherWin->requestFocus();
		});
	patGenBtn->setCallback([patGenWin](void) {
		bool visible = !patGenWin->visible();
		patGenWin->setVisible(visible);
		patGenWin->requestFocus();
		});
	gui->setVisible(true);
	gui->performLayout();
}
...
void Viewer::initEtcherMesh() {
	if (!state) return;

	state->etcherMesh = std::make_shared<RenderMesh>();
	if (!state->etcherMesh) return;

	float halfX = static_cast<float>(etcher.radius);
	float halfY = static_cast<float>(etcher.radius);
	float halfZ = static_cast<float>(etcher.length / 2.0);


	glm::vec3 v0(-halfX, -halfY, -halfZ);
	glm::vec3 v1(halfX, -halfY, -halfZ);
	glm::vec3 v2(halfX, halfY, -halfZ);
	glm::vec3 v3(-halfX, halfY, -halfZ);
	glm::vec3 v4(-halfX, -halfY, halfZ);
	glm::vec3 v5(halfX, -halfY, halfZ);
	glm::vec3 v6(halfX, halfY, halfZ);
	glm::vec3 v7(-halfX, halfY, halfZ);
	...
	state->etcherMesh->positions = std::make_shared<Buffer>((int)positions.size());
	state->etcherMesh->colors = std::make_shared<Buffer>((int)colors.size());

	for (size_t i = 0; i < positions.size(); ++i) {
		state->etcherMesh->positions->buffer[i] = positions[i];
		state->etcherMesh->colors->buffer[i] = colors[i];
	}


	state->etcherMesh->update({ POSITION, COLOR });
	std::cerr << "Etcher positions: " << state->etcherMesh->positions->buffer.size() << std::endl;
}
void Viewer::initTransforms() {
	...
	// bind all three programs' Transform block to binding point 1
	...
	// create transform buffer with space for 4 mat4 (projection, view, view3D, model)
...
void Viewer::updateEtcherMesh() {
	...
	if (!state->etcherMesh) initEtcherMesh();
	if (!state->etcherMesh) return;

	// Compute rotation matrices from etcher.rotation
	glm::mat4 rotX = glm::rotate<float>(glm::mat4(1.0f),
		static_cast<float>(etcher.rotation.x),
		glm::vec3(1.0f, 0.0f, 0.0f));

	glm::mat4 rotY = glm::rotate<float>(glm::mat4(1.0f),
		static_cast<float>(etcher.rotation.y),
		glm::vec3(0.0f, 1.0f, 0.0f));

	glm::mat4 rotZ = glm::rotate<float>(glm::mat4(1.0f),
		static_cast<float>(etcher.rotation.z),
		glm::vec3(0.0f, 0.0f, 1.0f));

	// Translation matrix from etcher.position
	glm::mat4 translate = glm::translate<float>(glm::mat4(1.0f),
		glm::vec3(
			static_cast<float>(etcher.position.x),
			static_cast<float>(etcher.position.y),
			static_cast<float>(etcher.position.z)
		));

	// Centering transform to move the box so its tip/center aligns with origin.
	glm::mat4 center = glm::translate<float>(glm::mat4(1.0f),
		glm::vec3(0.0f, 0.0f, static_cast<float>(-etcher.length / 2)));

	// Apply uniform scale to shrink model by 1/20.
	glm::mat4 scale = glm::scale(glm::mat4(1.0f), glm::vec3(ETCHER_SCALE));

	// Compose transform: translate * rotations * scale * center
	// NOTE: scale is applied in model space so geometry is uniformly smaller.
	state->etcherTransform = translate * rotZ * rotY * rotX * scale * center;
...
void Viewer::initModel()
{
	...
	if ((loadedModel = MeshIO::read(objPath, model, error))) {
		...
		std::cout << "Model loaded : meshes number " << model.meshes.size() << std::endl;
		std::cout << "vertices number  " << Viewer::countTotalVertices() << std::endl;
		for (int i = 0; i < model.size(); i++) {
...
void Viewer::updateUniforms(int index, int width, int height)
{
	...
	if (index == 0 || (index == 1 && (plotType == PlotType::shaded || plotType == PlotType::normals))) {
		...
		glUniform1f(glGetUniformLocation(modelShader.program, "innerR"), innerR);
		glUniform1f(glGetUniformLocation(modelShader.program, "outerR"), outerR);
		glUniform1f(glGetUniformLocation(modelShader.program, "ringSpacing"), ringSpacing);
	}
...
int Viewer::countTotalVertices(){
	int t = 0;
	// 遍历模型中的每个网格
	for (const auto& mesh : model.meshes) {
		t += mesh.vertices.size();
	}
	return t;
}
...
void Viewer::drawRenderMeshes(const ViewPane& pane, int index)
{
	...
	if (pane.left == surfacePane.left && state && state->etcherMesh) {
		// Save GL state
		...
		// Disable culling for etcher so it is visible from both sides (optional)
		glDisable(GL_CULL_FACE);
